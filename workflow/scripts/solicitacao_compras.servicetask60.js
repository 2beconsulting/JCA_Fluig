function servicetask60(attempt, message) { // GERAR PEDIDO DE COMPRAS
    ...
    try{
    	var pedidos = [];
        var limpeza = tools.pedido.limpaCotacao();
        
        log.info("limpeza%%%%%%%%%%%%")
        log.dir(limpeza)
        if(limpeza.ok){
        	var atualizaCotacao = tools.pedido.atualizaCotacao();
        	
        	log.info("atualizaCotacao%%%%%%%%%%%%")
            log.dir(atualizaCotacao)
        	if(atualizaCotacao.ok){ 
        		try{
            		var itens = tools.pedido.getItens();
                	if(itens.length > 0){
                		var obj = {
                			"COTACAO":[{
                    	...
                    	//log.dir(ret);
                    	if(ret.ok){
                    		var a = tools.pedido.registra(ret.retorno[1]);
