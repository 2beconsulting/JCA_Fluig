var tools = {
	aprovacao : {
		...
		carregaIntrack: function(){
			...
			var ciclo_atual = hAPI.getCardValue("ciclo_atual");
			var aprovsForm = hAPI.getChildrenIndexes("tabAprovacoesIntrack").filter(function(idx){return hAPI.getCardValue("aprovIntrack_ciclo___"+idx) == ciclo_atual}).map(function(idx){return {
	...
	cotacao: {
		...
		geraCicloInicial: function(cotacao_numero){
			log.info(">> tools.cotacao.geraCicloInicial [#550]")
			var obj = {
				"ok" : false,
				"errorMessage" : ""
			}
			...
			log.info(">> tools.cotacao.geraCicloInicial [#788]")
			...
			try{
			    ...
			    if(hAPI.getCardValue("tipoSc") != "5"){ // Tipo de Solicitação diferente de regularização 
					...
					if(obj.dados.length > 0){
						...
						var tabTES = hAPI.getChildrenIndexes("tabTES").map(function(idx){return {
							"idx"			: idx,
						...
						var tabFornecedor = hAPI.getChildrenIndexes("tabFornecedor").map(function(idx){return {
							"idx"			: idx,
							"A2_COD"		: hAPI.getCardValue("A2_COD"+"___"+idx),
							"A2_LOJA"		: hAPI.getCardValue("A2_LOJA"+"___"+idx),
					...
					obj.TES = hAPI.getChildrenIndexes("tabTES").filter(function(idx){return hAPI.getCardValue("TES_CODIGO"+"___"+idx) != ""}).map(function(idx){return {
						"idx"			: idx,
						"TES_A2_COD"	: ""+hAPI.getCardValue("TES_A2_COD"+"___"+idx),
						"TES_A2_LOJA"	: ""+hAPI.getCardValue("TES_A2_LOJA"+"___"+idx),
						"TES_B1_COD"	: ""+hAPI.getCardValue("TES_B1_COD"+"___"+idx)
					...
					obj.fornec = hAPI.getChildrenIndexes("tabFornecedor").filter(function(idx){return hAPI.getCardValue("CICLO_REMOVIDO"+"___"+idx) == ""}).map(function(idx){return {
						"A2_COD"		: hAPI.getCardValue("A2_COD"+"___"+idx),
						"A2_LOJA"		: hAPI.getCardValue("A2_LOJA"+"___"+idx),
						"A2_CGC"		: hAPI.getCardValue("A2_CGC"+"___"+idx),
						"A2_COND"		: hAPI.getCardValue("A2_COND"+"___"+idx),
			...
			var obj = [];
				...
				tabFornecedor.forEach(function(el){
					tabSC.forEach(function(it){
						log.dir(it);
						log.info(">> it.C1_PRODUTO: " + it.C1_PRODUTO + " > it.C1_PRODUTO.length: " + it.C1_PRODUTO.length)
	...
	pedido: {
		decisao:{
			aprovar: function(){
				...
				if(pedidos != ""){
					...
					var dadosProtheus = {"PEDIDO": pedidos.split(",")
						.map(function(pedido){return {
							"EMPRESA" 	: hAPI.getCardValue("idEmpresa"),
							"NUMERO" 	: pedido,
							"LIBERAR"	: "S",
							"APROVADOR" : aprovador
		...
		getItens: function(){
			...
			if(hAPI.getCardValue("tipoSc") != "5"){
				...
				var dsItens = hAPI.getChildrenIndexes("tabCotacao").map(function(idx){return {
					C8_ITEM 	: hAPI.getCardValue("C8_ITEM" + "___" + idx),
					C8_FORNECE 	: hAPI.getCardValue("C8_FORNECE" + "___" + idx),
					C8_LOJA 	: hAPI.getCardValue("C8_LOJA" + "___" + idx)
				}})
			...
			var itens = dsItens.map(function(item){return {
		...
		atualizaCotacao: function(){
			...
			if(hAPI.getCardValue("tipoSc") != "5"){
				...
				var cotacoes = hAPI.getChildrenIndexes("tabCotacao").map(function(idx){return {
					C8_PRODUTO 		: hAPI.getCardValue("C8_PRODUTO"+"___"+idx),
					C8_FORNECE 		: hAPI.getCardValue("C8_FORNECE"+"___"+idx),
					C8_LOJA 		: hAPI.getCardValue("C8_LOJA"+"___"+idx),
		...
		limpaCotacao: function(){
			...
			if(tipoSc != "5"){
				...
				if(cotacao.ok){
					var tabCotacao = hAPI.getChildrenIndexes("tabCotacao").map(function(idx){return {
	...
	tes:{
		...
		regulariza: function(){
			...
			var tes = hAPI.getChildrenIndexes("tabTES").map(function(idx){return {
				"idx"			: idx,
				"TES_A2_COD"	: ""+hAPI.getCardValue("TES_A2_COD"+"___"+idx),
				"TES_A2_LOJA"	: ""+hAPI.getCardValue("TES_A2_LOJA"+"___"+idx),
				"TES_A2_CGC"	: ""+hAPI.getCardValue("TES_A2_CGC"+"___"+idx),
