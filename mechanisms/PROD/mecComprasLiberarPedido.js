...
function mecComprasLiberarPedido(){
	...
	if(ds != null && ds.rowsCount > 0){
		...
		pre_alcada = pre_alcada.filter(function(el){
			log.info("valorPedido: " + valorPedido+" Limite: "+el.valorLimite);
			return el.valorLimite >= valorPedido
			}).sort(function(a,b){return a.valorLimite - b.valorLimite});
...
function valorTotal() {
	...
	hAPI.getChildrenIndexes("tabPedidos").forEach(function (idx) {
		valor += (toFloat(hAPI.getCardValue("C7_TOTAL___" + idx)))
	})
	return valor;
}