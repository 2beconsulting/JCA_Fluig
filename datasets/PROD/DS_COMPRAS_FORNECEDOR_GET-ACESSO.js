...
function createDataset(fields, constraints, sortFields) {
	try {
		...
		if (jsonConstraints.A2_COD != undefined && jsonConstraints.A2_LOJA != undefined) {
			...
			log.info("novoacesso: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
			log.dir(dsAcessos)
			...
			if (dsAcessos != null && dsAcessos.rowsCount > 0) {
				...
				log.info("novoacesso|dadosFornec: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
				log.dir(dadosFornec)
				if (dadosFornec != null) {
					var dadosOutro = tools.outroFornec(dadosFornec[0]["EMAIL"]);

					log.info("novoacesso|dadosOutro: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
					log.dir(dadosOutro)
					if (dadosOutro.rowsCount > 0 && dadosOutro.values.length > 0) {
						var fornAdic = tools.adicionaAcesso(dadosOutro.getValue(0, "DOCUMENTID"), dadosFornec);

						log.info("novoacesso|fornAdic: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
						log.dir(fornAdic)
						if (fornAdic.ok) {
						...
						log.info("novoacesso|dadosNovo: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
						log.dir(dadosNovo)
						if (dadosNovo.ok) {
							var ds = DatasetBuilder.newDataset();
							ds.addColumn("emailRegister");
					...
					log.info("novoacesso|erro: " + jsonConstraints.A2_COD + ":" + jsonConstraints.A2_LOJA)
		...
		log.info("novoacesso|erro ao procssar")
		log.info(e)
...
var tools = {
	adicionaAcesso: function (documentId, dadosFornec) {
		...
		log.info("novoacesso|adicionaAcesso: " + dadosFornec[0]["CODIGO"] + ":" + dadosFornec[0]["LOJA"])
		log.dir(dadosFornec)
	...
	novoAcesso: function (dadosFornec) {
		...
		if (cardCreated.ok) {
			...
			log.info("novoacesso|novoAcesso|cardCreated.ok: " + dadosFornec[0]["CODIGO"] + ":" + dadosFornec[0]["LOJA"])
			log.dir(dadosFornec)
			if (cardCreated.retorno.cardId != undefined) {
				...
				if (child.ok) {
					...
					log.info("novoacesso|novoAcesso|child.ok: " + dadosFornec[0]["CODIGO"] + ":" + dadosFornec[0]["LOJA"])
					log.dir(dadosFornec)
	...
	outroFornec: function (email) {
		return integra.Exec(
			"SELECT  ML.DOCUMENTID, ML.emailRegister FROM " + mlAutenticator + " ML \
				INNER JOIN DOCUMENTO DOC ON DOC.NR_DOCUMENTO = ML.DOCUMENTID AND DOC.NR_VERSAO = ML.VERSION \
...
var integra = {
	...
	postFluig: function (endpoint, params) {
		...
		try {
			...
			log.info("novoacesso|.putFormFluig **");
			log.info(e);
